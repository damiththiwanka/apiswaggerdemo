import 'package:flutter/material.dart';
import 'package:base3/src/values/colors.dart' as colors;

import 'card_view.dart';
import 'text.dart';

class CustomNavBottomSheet extends StatelessWidget{
  final String buttonLeft;
  final String buttonRight;
  final Function actionButtonLeft;
  final Widget pageToCall;

  const CustomNavBottomSheet({Key? key, required this.buttonLeft, required this.buttonRight, required this.actionButtonLeft, required this.pageToCall}) : super(key: key);

  @override
  Widget build(BuildContext context) {
    return SafeArea(
      child: CardView(
          width: double.infinity,
          height: 63,
          backgroundColor: colors.AppColors.primaryColor,
          elevation: 4,
          alignment: Alignment.center,
          noCorner: true,
          margin: EdgeInsets.all(10),
          child: ListTile(
            leading: Padding(
              padding: const EdgeInsets.only(top: 16),
              child: GestureDetector(
                onTap: ()=> actionButtonLeft(context),
                child: CustomText(
                  text: buttonLeft,
                ),
              ),
            ),
            trailing: Padding(
              padding: const EdgeInsets.only(top:16.0),
              child: TextButton(
                  onPressed: (){
                    Navigator.push(context, MaterialPageRoute(builder: (context)=> pageToCall));},
                  child: CustomText(
                      color: colors.AppColors.accentColor,
                      text: buttonRight
                  )
              ),
            ) ,
          )
      ),
    );
  }
}