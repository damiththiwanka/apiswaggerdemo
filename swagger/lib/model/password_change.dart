...
class PasswordChange {
  ...
  static List<PasswordChange> listFromJson(List<dynamic> json) {
    return json == null
        ? new List<PasswordChange>()
        : json.map((value) => new PasswordChange.fromJson(value)).toList();
  }

  static Map<String, PasswordChange> mapFromJson(
      Map<String, Map<String, dynamic>> json) {
    var map = new Map<String, PasswordChange>();
    if (json != null && json.length > 0) {
      json.forEach((String key, Map<String, dynamic> value) =>
          map[key] = new PasswordChange.fromJson(value));
    }
    return map;
  }
}
